...
def get_args():
    parser = argparse.ArgumentParser(description='Stores PMC OA articles (S3/JATS) in sqlite database', add_help=True)
    parser.add_argument('--database', dest='db', required=True)
    parser.add_argument('--pmcid_list', dest='pmcid_file', required=True,
                        help='Text file with one PMCID per line, e.g. PMC1234567')
    parser.add_argument('--doi_to_keep', dest='doi_file', required=False)
    parser.add_argument('--min_year', dest='min_year', type=int, required=False)
    parser.add_argument('--log_file', dest='log_file', required=False)
    parser.add_argument('--buffer_size', dest='buffer_size', type=int, default=400)
    args = parser.parse_args()
    return (args.db, args.pmcid_file, args.doi_file or '', args.min_year,
            args.log_file or '', args.buffer_size)
...
def extract_sections(root):
    ...
    if abstract_el is not None:
        abs_text = ' '.join(t.strip() for t in abstract_el.itertext() if t.strip())
    ...
    if body is not None:
        ...
        if secs:
            for sec in secs:
                ...
                title_text = title_el.text.strip() if title_el is not None and title_el.text else sec_type
                paras = sec.findall('.//p')
                sec_text = ' '.join(t.strip() for p in paras for t in p.itertext() if t.strip())
                if sec_text:
                    body_text += f"{title_text.upper()}\n\n{sec_text}\n\n" if title_text else f"{sec_text}\n\n"
        else:
            # body sem <sec>: pega parágrafos direto do body
            paras = body.findall('p')
            body_text = ' '.join(t.strip() for p in paras for t in p.itertext() if t.strip())
    return abs_text, body_text

db, pmcid_file, doi_list_file, min_year, log_file, max_buffer_size = get_args()
...
report_stats = {"missing_pmid": 0, "missing_doi": 0, "normal_cases_pmid": 0,
                "normal_cases_doi": 0, "filtered_doi_prefix": 0, "not_found": 0}
data_buffer = []
...
for pmcid in pmcids:
    ...
    exists = db_cursor.execute('SELECT 1 FROM pcw_literature WHERE pmid = ?', (pmid,)).fetchone()
            ...
            for doi_journal in doi_list_to_keep.get(prefix, []):
                if suffix.startswith(doi_journal):
                    try:
                        ...
                        time.sleep(0.5)
                        journal_name = works_res['container-title'][0] if works_res and 'container-title' in works_res else ''
                    except Exception:
                        journal_name = ''
                    report_stats["filtered_doi_prefix"] += 1
                    data_buffer.append((pmid, title, year, doi, journal_name, first_author, abs_text, article_text))
                    break
    else:
        journal_name = meta.get('citation', '')
        data_buffer.append((pmid, title, year, doi, journal_name, first_author, abs_text, article_text))

    if report_stats['normal_cases_pmid'] % 1000 == 0 and report_stats['normal_cases_pmid'] != 0:
...
if log_file:
    with open(log_file, "a") as lf:
        lf.write(json.dumps(report_stats) + "\n")
